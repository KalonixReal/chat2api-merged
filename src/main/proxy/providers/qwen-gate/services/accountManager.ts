...
import { scheduleAutomaticAccountRecovery, type AccountRecoveryTrigger } from './accountRecovery.ts';
import { publishCaptchaAlert, publishManualLoginAlert, publishThrottleAlert } from './captchaAlertService.ts';
...
export async function addAccount(email: string, password: string): Promise<{ loginSucceeded: boolean; loginError?: string }> {
  ...
  if (profileResult === 'captcha') {
    publishCaptchaAlert(normalizedEmail, 'Login CAPTCHA requires manual verification in the Accounts dashboard', 'login');
    logStore.log('info', 'account', `Captcha for ${normalizedEmail} — waiting for manual login in the Accounts dashboard`);
  }
  ...
  if (newState) {
    ...
    if (profileResult !== 'captcha') {
      publishManualLoginAlert(normalizedEmail, 'Automatic sign-in failed. Open Accounts and click Login.', 'login');
    }
...
export function throttleAccount(email: string, durationMs?: number, recoveryTrigger: AccountRecoveryTrigger = 'throttled'): void {
  const acct = getAccountByEmail(email);
  if (!acct) return;
  ...
  publishThrottleAlert(email, acct.throttledUntil, `Qwen throttled this account until ${unlockTime}.`);
  // Persist so restart respects the cooldown
  saveAccountsToFile(accounts);
  scheduleAutomaticAccountRecovery(acct, recoveryTrigger);
}
