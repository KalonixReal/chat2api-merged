...
export async function openBrowserProfile(email: string, password?: string, options?: BrowserProfileOptions): Promise<LoginResult> {
  ...
  try {
    ...
    if (result) {
      if (result === 'captcha') {
        logStore.log('warn', 'browser', `CAPTCHA detected for ${email}; closing the automatic browser before manual login`);
        try {
          await context.close();
        } catch {
          logStore.log('warn', 'browser', `context.close failed after CAPTCHA detection for ${email}`);
        }
