...
import { attemptAutomaticAccountRecovery } from './accountRecovery.ts';
import {
  accounts,
  decodeJwt,
...
export async function initAuth(onAccountReady?: (email: string) => Promise<void>): Promise<void> {
  ...
  if (merged.length === 0) {
    ...
    logStore.log(
      'warn',
      'auth',
      'No saved accounts found. Use the Provider Setup page in the dashboard to add accounts (browser login or email + password).',
    );
    return;
  }
  ...
  try {
    ...
    if (needLogin.length > 0) {
      ...
      for (let i = 0; i < needLogin.length; i += MAX_CONCURRENT_PROFILE_LOADS) {
        ...
        await Promise.allSettled(
          batch.map(async (acct) => {
            await attemptAutomaticAccountRecovery(acct, 'startup');
          }),
        );
      }
    }
    ...
    for (const acct of accounts) {
      if (!acct.disabled && !acct.state?.token && !acct.password) {
        publishManualLoginAlert(
          acct.email,
          'Automatic sign-in cannot run because no password is stored. Open Accounts and update this account.',
          'login',
        );
      }
    }
  ...
  try {
    ...
    if (!existsSync(join(profileDir, 'Default', 'Cookies'))) {
      ...
      if (password) {
        ...
        if (result === 'captcha') {
          publishCaptchaAlert(email, 'Login CAPTCHA requires manual verification in the Accounts dashboard', 'login');
          logStore.log('info', 'auth', `Captcha for ${email} — waiting for manual login in the Accounts dashboard`);
        }
        if (result === 'success') {
