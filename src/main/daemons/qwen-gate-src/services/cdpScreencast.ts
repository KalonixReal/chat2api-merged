...
import { findChromeBinary } from './chromeBinary.ts';
...
export interface ScreencastInputEvent {
  type: string;
  x: number;
  y: number;
  button?: number;
...
export async function startScreencast(
  ...
  const chromeBin = findChromeBinary(config.get('BROWSER'));
  if (!chromeBin) {
    const error =
      'No Chromium-based browser found. Install Chrome or Edge, or set CHROME_BIN to the full browser executable path.';
    logStore.log('error', 'screencast', error);
    return { error };
  }
...
async function connectCDP(session: ScreencastSession, wsUrl: string): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    ws.on('message', (data: Buffer) => {
      ...
      // Reaching the app is a useful signal, but do not close the browser yet.
      // The token cookie can arrive slightly later; login polling persists it
      // before telling the dashboard that authentication is complete.
      if (msg.method === 'Page.frameNavigated') {
        ...
        if (url && !url.startsWith('about:') && url.includes('chat.qwen.ai') && !url.includes('/auth')) {
          logStore.log('info', 'screencast', `Post-auth page reached for ${session.email} — waiting for token cookie`);
        }
      }
    });
  ...
  session.loginCheckInterval = setInterval(async () => {
    ...
    try {
      ...
      if (tokenCookie && tokenCookie.expires && tokenCookie.expires * 1000 > Date.now()) {
        if (session.loginCheckInterval) {
          clearInterval(session.loginCheckInterval);
          session.loginCheckInterval = null;
        }
...
async function cdpSend(session: ScreencastSession, method: string, params?: any): Promise<any> {
  return new Promise((resolve, reject) => {
    ...
    const message: { id: number; method: string; params?: any; sessionId?: string } = { id, method, params };
    const pageSessionId = (session.cdpWs as any)._pageSessionId;
    if (pageSessionId && !method.startsWith('Target.')) {
      message.sessionId = pageSessionId;
    }
    session.cdpWs.send(JSON.stringify(message));
  });
}
...
export function getCdpMouseState(
  eventType: 'mousemove' | 'mousedown' | 'mouseup',
  button = 0,
  buttons = 0,
): { button: 'none' | 'left' | 'middle' | 'right'; buttons: number } {
  if (eventType === 'mousemove') {
    if (buttons & 1) return { button: 'left', buttons };
    if (buttons & 2) return { button: 'right', buttons };
    if (buttons & 4) return { button: 'middle', buttons };
    return { button: 'none', buttons: 0 };
  }

  const cdpButton = button === 2 ? 'right' : button === 1 ? 'middle' : 'left';
  return { button: cdpButton, buttons };
}

export function handleInputEvent(email: string, event: ScreencastInputEvent): void {
  const session = sessions.get(email);
  ...
  function send(method: string, params: any) {
    cdp.send(JSON.stringify({ id: session!.inputMsgId++, method, params, sessionId }));
  }
  ...
  switch (event.type) {
    ...
    case 'mousemove':
      send('Input.dispatchMouseEvent', {
        ...
        x: event.x,
        y: event.y,
        ...getCdpMouseState('mousemove', event.button, event.buttons),
      });
      break;
    case 'mousedown':
      send('Input.dispatchMouseEvent', {
        ...
        x: event.x,
        y: event.y,
        ...getCdpMouseState('mousedown', event.button, event.buttons ?? 1),
        clickCount: 1,
      });
      break;
    case 'mouseup':
      send('Input.dispatchMouseEvent', {
        ...
        x: event.x,
        y: event.y,
        ...getCdpMouseState('mouseup', event.button, event.buttons ?? 0),
        clickCount: 1,
      });
      break;
    case 'keydown': {
