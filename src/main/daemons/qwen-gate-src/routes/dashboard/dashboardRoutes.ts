...
import { getAccountAlerts, getCaptchaAlerts } from '../../services/captchaAlertService.ts';
...
function captchaAlertsHandler(c: any) {
  const parsed = Number(c.req.query('after') || 0);
  const afterId = Number.isFinite(parsed) && parsed > 0 ? parsed : 0;
  return c.json({ alerts: getCaptchaAlerts(afterId) });
}

function accountAlertsHandler(c: any) {
  const parsed = Number(c.req.query('after') || 0);
  const afterId = Number.isFinite(parsed) && parsed > 0 ? parsed : 0;
  return c.json({ alerts: getAccountAlerts(afterId) });
}
    ...
    async start(controller) {
      ...
      for (const email of emails) {
        try {
          ...
          sessionPool.forgetConversations(email);
          deleted++;
...
export function registerDashboardRoutes(app: Hono): void {
  ...
  app.get('/dashboard/account-alerts', async (c, next) => requireApiKey(c, next), accountAlertsHandler);
  app.get('/dashboard/captcha-alerts', async (c, next) => requireApiKey(c, next), captchaAlertsHandler);
