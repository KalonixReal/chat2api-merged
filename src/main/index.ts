...
async function initializeApp(): Promise<void> {
  ...
  app.on('before-quit', () => {
    app.isQuitting = true
    trayManager?.destroy()
    // Stop the vendored daemons (qwen-gate / deepseek-api / glm-free-api /
    // kimi-free-api). The supervisor never throws.
    daemonSupervisor.stopAll().catch((err) => {
      console.error('[App] Daemon supervisor stop failed:', err)
    })
  })
