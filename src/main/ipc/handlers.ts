...
export async function registerIpcHandlers(mainWindow: BrowserWindow | null): Promise<void> {
  ...
  // Spawn the 4 vendored daemons (qwen-gate, deepseek-api, glm-free-api,
  // kimi-free-api). They are independent of the proxy server — even if the
  // proxy is not running, we still spawn them so a later proxy start has the
  // daemons ready. The supervisor never throws.
  //
  // Auto-bootstrap: if install.sh hasn't been run yet (first launch), run it
  // silently before spawning daemons. This makes `bun start` a true single
  // entry point — no manual install step required.
  try {
    ...
    // Subscribe to the smart switcher (Task 6) — dynamic import, try/catch.
