...
import { publishCaptchaAlert } from './captchaAlertService.ts';
...
export async function createQwenStream(
  ...
  async function handleErrorResponse(response: Response, debugEntryId: string): Promise<never> {
    ...
    if (contentType.includes('application/json')) {
      try {
        ...
        if (errorJson?.success === false) {
          ...
          const isAuthenticationFailure = /unauthori[sz]ed|unauthenticated|invalid[\s_-]*token|token[\s_-]*expired|login[\s_-]*required|not[\s_-]*logged[\s_-]*in/i.test(
            `${code} ${details}`,
          );
          if (isAuthenticationFailure && currentAccountEmail) {
            const failedEmail = currentAccountEmail;
            throttleAccount(failedEmail, 5 * 60 * 1000, 'auth_error');
            const nextAccount = await pickAccount(failedEmail);
            if (nextAccount) {
              currentAccountEmail = nextAccount.email;
              decrementInFlight(nextAccount.email);
            }
            throw new RetryableQwenStreamError(
              `Qwen authentication failed for ${failedEmail}; automatic re-login started`,
              3000,
            );
          }
          if (code === 'RateLimited' && currentAccountEmail) {
            ...
            throttleAccount(currentAccountEmail, throttleMs, 'rate_limit');
        ...
        if (errorJson?.ret?.[0] === 'FAIL_SYS_USER_VALIDATE') {
          ...
          publishCaptchaAlert(currentAccountEmail || 'unknown', details, 'request');
          if (currentAccountEmail) {
            throttleAccount(currentAccountEmail, 5 * 60 * 1000, 'captcha');
            logStore.log(
              'debug',
              'qwen',
    ...
    if ((response.status === 401 || response.status === 403) && currentAccountEmail) {
      const failedEmail = currentAccountEmail;
      throttleAccount(failedEmail, 5 * 60 * 1000, 'auth_error');
      const nextAccount = await pickAccount(failedEmail);
      if (nextAccount) {
      ...
      throw new RetryableQwenStreamError(`Qwen authentication failed for ${failedEmail}; automatic re-login started`, 3000);
    }
  ...
  // Qwen sometimes reports an error as HTTP 200 + JSON instead of SSE. If
  // this JSON is piped into the SSE parser, the client sees an empty model
  // turn. Route it through the existing upstream error handler instead.
  const responseContentType = result.response.headers.get('content-type') || '';
