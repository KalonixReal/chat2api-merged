...
async function setupAnthropicSession(
  ...
  for (let attempt = 0; attempt < MAX_ACCOUNT_RETRIES; attempt++) {
    ...
    logStore.log(
      'debug',
      'chat',
      `[Anthropic] Attempt ${attempt + 1}/${MAX_ACCOUNT_RETRIES} account=${accountEmail || 'NONE'} continued=${!!session.reused} lastFailed=${lastFailedEmail || 'none'}`,
    );
...
async function handleAnthropicStream(
  ...
  return honoStream(c, async (streamWriter: any) => {
    ...
    try {
      ...
      sessionPool.release(session.chatId, targetResponseId || nextParentId, sessionHeaders, resolvedEmail);
    } catch (streamErr: any) {
