...
async function setupSession(messages: any[], body: OpenAIRequest, availableTokens: number, toolCalling: boolean, logId: string) {
  ...
  // Message preparation happens after conversation acquisition so continued
  // website chats receive only the new turn.
  ...
  for (let attempt = 0; attempt < MAX_ACCOUNT_RETRIES; attempt++) {
    let session;
    try {
      session = await sessionPool.acquireForConversation(cleanedMessages, lastFailedEmail);
    } catch (err: any) {
      lastError = err;
      lastFailedEmail = err?.accountEmail || lastFailedEmail;
      logStore.addError(logId, `Session acquire failed: ${err?.message || String(err)}`);
      if (/All accounts are rate-limited or unavailable/i.test(err?.message || '')) throw lastError;
      continue;
    }
    ...
    const resolvedEmail = accountEmail;

    let prepared;
    try {
      prepared = prepareQwenTurn(cleanedMessages, body, availableTokens, toolCalling, !!session.reused);
    } catch (err) {
      sessionPool.release(session.chatId, nextParentId, sessionHeaders, resolvedEmail, false);
      throw err;
    }
    const { qwenMessages: processedMessages, systemContent, toolResultsContent, chatHistoryContent } = prepared;
    ...
    const sessionMessages = applySessionCorrections(session, processedMessages, accountEmail);
    ...
    try {
      ...
      if (
        ...
        lastFailedEmail = resolvedEmail;
        lastError = err;
        if (resolvedEmail) throttleAccount(resolvedEmail, 5 * 60 * 1000, 'captcha');
        continue;
      }
...
export async function chatCompletions(c: Context) {
  ...
  try {
    ...
    const { sessionMessages, session, nextParentId, sessionHeaders, resolvedEmail, stream, qwenAbortController, routedModel, isThinkingModel } = await setupSession(
      messages,
      body,
