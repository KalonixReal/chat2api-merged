...
import { type AmplificationGuardState, createQwenStreamWithRetry, type QwenMessage } from './chatHelpers.ts';
...
import { detectIncompleteAgentCompletion, FALSE_COMPLETION_RETRY_PROMPT } from './falseCompletionGuard.ts';
...
export interface StreamingContext {
  ...
  toolCalling: boolean;
  cleanOutput: boolean;
  sessionMessages: QwenMessage[];
  routedModel: string;
  isThinkingModel: boolean;
  qwenLogFile?: string;
}

function buildFalseCompletionRetryMessage(baseMessages: QwenMessage[], parentId: string | null): QwenMessage {
  const base = baseMessages[baseMessages.length - 1];
  ...
  return {
    ...
    childrenIds: [],
    role: 'user',
    content: FALSE_COMPLETION_RETRY_PROMPT,
    files: [],
    timestamp,
    user_action: 'chat',
  };
...
function prepareStateForFalseCompletionRetry(state: StreamProcessingState): void {
  if (state.pendingChunk) {
    state.lastRawContent += state.pendingChunk;
    state.lastFullContent += state.pendingChunk;
    state.pendingChunk = '';
  }
  state.targetResponseId = null;
  state.currentThoughtIndex = 0;
  state.lastVStrRaw = '';
...
export async function handleStreamingRequest(ctx: StreamingContext): Promise<Response> {
  ...
  return honoStream(c, async (streamWriter: any) => {
    ...
    try {
      ...
      let loopResult = await runStreamLoop(c, reader, streamState, streamCtx, ampState, bufferRef);
      ...
      const contentBeforeRetry = streamState.lastFullContent + streamState.pendingChunk;
      const incompleteCompletionReason =
        !streamState.upstreamError &&
        detectIncompleteAgentCompletion({
          content: contentBeforeRetry,
          hasAvailableTools: !!body.tools?.length,
          toolCallCount: streamCtx.emittedToolCallCount,
        });

      if (incompleteCompletionReason) {
        logStore.log(
          'warn',
          'chat',
          `[Chat] ${incompleteCompletionReason} completion detected for ${logId}; continuing the same Qwen chat once instead of returning stop`,
        );
        logStore.addError(logId, `${incompleteCompletionReason} completion auto-retried once`);
        prepareStateForFalseCompletionRetry(streamState);

        try {
          const retryMessage = buildFalseCompletionRetryMessage(ctx.sessionMessages, streamState.nextParentId);
          const retryResult = await createQwenStreamWithRetry(
            [retryMessage],
            ctx.isThinkingModel,
            ctx.routedModel,
            session.chatId,
          ...
          streamCtx.qwenAbortController = retryResult.abortController;
          streamCtx.qwenLogFile = retryResult.qwenLogFile;
          reader = retryResult.stream.getReader();
          streamReader = reader;
          const retryBufferRef = { text: '' };
          loopResult = await runStreamLoop(c, reader, streamState, streamCtx, ampState, retryBufferRef);
          if (loopResult.error) {
            logStore.addError(logId, `Action-only completion retry failed: ${loopResult.error}`);
          }
        } catch (retryError) {
          const message = retryError instanceof Error ? retryError.message : String(retryError);
          logStore.log('warn', 'chat', `[Chat] Action-only completion retry failed for ${logId}: ${message}`);
          logStore.addError(logId, `Action-only completion retry failed: ${message}`);
        }
      }
