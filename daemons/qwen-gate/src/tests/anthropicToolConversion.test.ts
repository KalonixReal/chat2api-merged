...
describe('Qwen local_mcp request budget', () => {
  test('keeps 69 tools while replacing oversized unknown descriptions without slicing', async () => {
    const { buildQwenMessages } = await import('../routes/chatHelpers.ts');
    const { UNMAPPED_TOOL_DESCRIPTION } = await import('../routes/toolDescriptionSummaries.ts');
    const tools = Array.from({ length: 69 }, (_, index) => ({
      type: 'function',
      function: {
        name: `tool_${index}`,
        description: `Tool ${index}: ${'long description '.repeat(250)}`,
        parameters: {
          type: 'object',
          properties: {
            value: { type: 'string', description: `Value for tool ${index}` },
          },
          required: ['value'],
        },
      },
    ...
    const { qwenMessages } = buildQwenMessages(
      [{ role: 'user', content: 'Analyze this project.' }],
      { model: 'qwen3.8-max', tools },
      32_000,
      true,
    );
    const localMcp = qwenMessages[0].feature_config.local_mcp;

    expect(Object.keys(localMcp['★'])).toHaveLength(69);
    expect(JSON.stringify(localMcp).length).toBeLessThanOrEqual(70_000);
    expect(localMcp['★'].tool_0.description).toBe(UNMAPPED_TOOL_DESCRIPTION);
    expect(localMcp['★'].tool_0.input_schema.required).toEqual(['value']);
    expect(localMcp['★'].tool_68.input_schema.properties.value.type).toBe('string');
  });

  test('never truncates the general-purpose subagent type literal', async () => {
    const { buildQwenMessages } = await import('../routes/chatHelpers.ts');
    const agentDescription = `Launch a subagent for independent work. ${'Long agent guidance. '.repeat(300)}`;
    const tools = [
      {
        type: 'function',
        function: {
      ...
      ...Array.from({ length: 68 }, (_, index) => ({
        ...
        function: {
          ...
          description: `Large tool ${index}. ${'Verbose guidance. '.repeat(250)}`,
          parameters: { type: 'object', properties: { value: { type: 'string' } } },
        },
      })),
    ];

    const { qwenMessages, systemContent } = buildQwenMessages(
      [{ role: 'user', content: 'Use a subagent.' }],
      { model: 'qwen3.8-max', tools },
      32_000,
      true,
    );
    const taskDescription = qwenMessages[0].feature_config.local_mcp['★'].Task.description;

    expect(taskDescription).toContain('general-purpose');
    expect(taskDescription).not.toMatch(/\bgeneral-purpos\b/);
    expect(taskDescription).toContain('fork');
    expect(systemContent).toContain('general-purpose');
    expect(JSON.stringify(qwenMessages[0].feature_config.local_mcp).length).toBeLessThanOrEqual(70_000);
  });
});
